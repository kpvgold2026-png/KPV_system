CREATE OR REPLACE FUNCTION create_tradein_tx(
  p_phone TEXT,
  p_bill_id TEXT,
  p_old_items JSONB,
  p_new_items JSONB,
...
GRANT EXECUTE ON FUNCTION create_tradein_tx(TEXT, TEXT, JSONB, JSONB, JSONB, TEXT, NUMERIC, NUMERIC, NUMERIC, NUMERIC, NUMERIC) TO authenticated;
...
BEGIN
  ...
  SELECT status INTO v_status FROM transactions WHERE id = p_tx_id AND type = 'TRADEIN';
...
BEGIN
  ...
  SELECT t.status, t.total, t.diff_amount, t.premium
  INTO v_status, v_total, v_diff_amount, v_premium
  ...
  INTO v_new_items FROM transaction_items
  WHERE tx_id = p_tx_id AND item_role = 'NEW';

  SELECT jsonb_agg(jsonb_build_object('productId', product_id, 'qty', qty))
  INTO v_old_items FROM transaction_items
  WHERE tx_id = p_tx_id AND item_role IN ('OLD', 'FOC');
  ...
  VALUES (p_tx_id, 'OLD', 'TRADEIN', 'IN', v_old_gold_g, FALSE, v_user_id, NOW())
  RETURNING id INTO v_move_id;
  ...
  LOOP
    ...
    IF v_role = 'Sales' OR v_role IS NULL THEN
      ...
      VALUES (v_user_id, v_item.product_id, v_item.qty, 'TRADEIN', p_tx_id, NOW(), v_user_id);
    END IF;
  END LOOP;
  ...
  VALUES (p_tx_id, 'TRADEIN', v_diff_amount, COALESCE(v_premium, 0), v_new_cost, v_diff, NOW())
