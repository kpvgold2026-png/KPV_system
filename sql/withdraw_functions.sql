CREATE OR REPLACE FUNCTION create_withdraw_tx(
  p_phone TEXT,
  p_bill_id TEXT,
  p_items JSONB,
  p_premium NUMERIC,
  p_total NUMERIC,
  p_withdraw_code TEXT,
...
GRANT EXECUTE ON FUNCTION create_withdraw_tx(TEXT, TEXT, JSONB, NUMERIC, NUMERIC, TEXT, NUMERIC) TO authenticated;
...
BEGIN
  ...
  END IF;
  SELECT status INTO v_status FROM transactions WHERE id = p_tx_id AND type = 'WITHDRAW';
